...
import { Dialog, DialogBody, DialogContent, DialogFooter, DialogHeader, DialoTrigger } from "../../../components/dialog";
                ...
                <div className="space-y-3">
                    ...
                    {!isLoadingPhotos && photos.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                            {photos.map((item)=>(
                               <PhotoImageSelectable key={item.id} src={`${import.meta.env.VITE_IMAGES_URL}/${item.imageId}`} title={item.title}
                               onSelectImage={(selected)=> handleTogglePhoto(selected, item.id)}
